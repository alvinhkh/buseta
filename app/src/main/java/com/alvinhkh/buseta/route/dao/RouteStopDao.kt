...
interface RouteStopDao {
    ...
    @Query("DELETE FROM route_stops WHERE company_code = :companyCode AND route_no = :routeNo AND route_sequence = :routeSequence AND route_service_type = :routeServiceType AND last_update < :lastUpdate")
    fun delete(companyCode: String, routeNo: String, routeSequence: String, routeServiceType: String, lastUpdate: Long): Int
    ...
    fun liveData(companyCode: String, routeNo: String, routeSequence: String, routeServiceType: String): LiveData<MutableList<RouteStop>>
